...
{
//__android_log_print(ANDROID_LOG_INFO, "BeepingCoreLibInfo", "EncoderAudibleMultiTone init" );
...
//  __android_log_print(ANDROID_LOG_INFO, "BeepingCoreLibInfo", "Added digits");
  ...
  for (int i = 0; i < digits.size(); i++)
  {
    ...
    if (i<(20-1))
      Globals::getFreqsFromIdxAudibleMultiTone(digits[i+1], mSampleRate, mWindowSize, (float**)&nextFreqs);
    else
    {
      nextFreqs[0] = 18000.f;
      nextFreqs[1] = 18000.f;
