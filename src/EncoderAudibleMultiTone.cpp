...
EncoderAudibleMultiTone::EncoderAudibleMultiTone(const BeepingConfig& config,
                                                 float samplingRate,
  ...
  BTRACE(
      "EncoderAudibleMultiTone::ctor sr={:.1f} buf={} win={} tokens={} "
      "tones={}",
...
int EncoderAudibleMultiTone::EncodeDataToAudioBuffer(const char* stringToEncode,
                                                     int type, int size,
                                                     const char* melodyString,
                                                     int melodySize) {
  BINFO("EncoderAudibleMultiTone::Encode payload=\"{}\" len={} type={}",
  ...
  BDEBUG("EncoderAudibleMultiTone::Encode RS encoded {} digits",
         (int)digits.size());
  ...
  for (int i = 0; i < static_cast<int>(digits.size()); i++) {
    ...
    Globals::getFreqsFromIdxAudibleMultiTone(
        digits[i], mSampleRate, mWindowSize, (float**)&mCurrentFreqs);
    Globals::getLoudnessAudibleMultiToneFromIdx(
    ...
    if (i > 1)  // after the start tokens
    {
      ...
      {
        mCurrentFreqs[0] += m_config.freqOffsetForAudibleMultiTone;
        mCurrentFreqs[1] += m_config.freqOffsetForAudibleMultiTone;
      } else if ((n % 3) == 2)  // Take care, encoder and decoder do not share
                                // same odd/even order if you compare mDecoding
                                // with digits index
      {
        mCurrentFreqs[0] += m_config.freqOffsetForAudibleMultiTone * 2;
        mCurrentFreqs[1] += m_config.freqOffsetForAudibleMultiTone * 2;
      }
    }
    ...
    if (i<(20-1))
      Globals::getFreqsFromIdxAudibleMultiTone(digits[i+1], mSampleRate,
    ...
    odd/even order if you compare mDecoding with digits index
      {
        nextFreqs[0] += m_config.freqOffsetForAudibleMultiTone;
        nextFreqs[1] += m_config.freqOffsetForAudibleMultiTone;
      }
      else if ((n % 3) == 2) //Take care, encoder and decoder do not share same
    odd/even order if you compare mDecoding with digits index
      {
        nextFreqs[0] += m_config.freqOffsetForAudibleMultiTone * 2;
        nextFreqs[1] += m_config.freqOffsetForAudibleMultiTone * 2;
      }
    }
...
*/
    ...
    BTRACE(
        "EncoderAudibleMultiTone::Encode digit[{}]={} freq1={:.2f} "
        "freq2={:.2f}",
  ...
  BDEBUG("EncoderAudibleMultiTone::Encode totalSamples={}",
      ...
      for (int t = 0; t < samplesPerDigit; t++) {
        ...
        mAudioBufferEncodedString[t + i * samplesPerDigit] =
            ...
            (vol * factor * sin(pos * 2.f * BeepingConfig::pi));
                         ...
                         // mNumSamplesEncodedString
  {
    BDEBUG("EncoderAudibleMultiTone::Encode applying melody, melodySize={}",
           melodySize);
  ...
  if (mnAudioSignatureSamples >
      0)  // Add Audio Signature (mAudioSignature) to mAudioBufferEncodedString
          // of size mNumSamplesEncodedString
  {
    int sizeToFill = mnAudioSignatureSamples;
    BDEBUG("EncoderAudibleMultiTone::Encode applied audio signature {} samples",
           sizeToFill);
