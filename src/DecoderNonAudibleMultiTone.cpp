...
DecoderNonAudibleMultiTone::DecoderNonAudibleMultiTone(
  ...
  Globals::getIdxsFromIdxNonAudibleMultiTone(idxFrontDoorToken1,
                                             &idxTonesFrontDoorToken1);
  idxTonesFrontDoorToken2 = new int[2];
  Globals::getIdxsFromIdxNonAudibleMultiTone(idxFrontDoorToken2,
  ...
  BDEBUG("DecoderNonAudibleMultiTone::ctor freq2Bin=%.6f beginBin=%d endBin=%d",
         mFreq2Bin, mBeginBin, mEndBin);
}

DecoderNonAudibleMultiTone::~DecoderNonAudibleMultiTone(void) {
  BTRACE("DecoderNonAudibleMultiTone::dtor");
  delete[] mFreqsBins;
  delete[] mIdxs;
...
int DecoderNonAudibleMultiTone::DecodeAudioBuffer(float* audioBuffer,
                                                  int size) {
  BTRACE("DecoderNonAudibleMultiTone::DecodeAudioBuffer size=%d decoding=%d",
         size, mDecoding);
  ...
  while (getSizeFilledFrameCircularBuffer() >=
    ...
    if (mDecoding == 0) {
      ...
      if (ret == 1) {
        ...
        BINFO(
            ...
            "DETECTED");
        BTRACE("DecoderNonAudibleMultiTone::DecodeAudioBuffer -> %d", -2);
                                             ...
                                             // now start decoding
    {
      ...
      if (ret >= 0) {
        ...
        BTRACE("DecoderNonAudibleMultiTone::DecodeAudioBuffer -> %d", ret);
        return ret;
      }
    } else if (mDecoding >
                                           ...
                                           // complete word
    {
      ...
      BINFO("DecoderNonAudibleMultiTone::DecodeAudioBuffer WORD COMPLETE");
      BTRACE("DecoderNonAudibleMultiTone::DecodeAudioBuffer -> %d", -3);
...
int DecoderNonAudibleMultiTone::GetDecodedData(char* stringDecoded) {
  ...
  BINFO("DecoderNonAudibleMultiTone::GetDecodedData -> rc=%d", result);
...
int DecoderNonAudibleMultiTone::AnalyzeStartTokens(
    float* audioBuffer) {  // float *magSpectrum, float* realSpectrum, float*
                           // imagSpectrum
  BTRACE("DecoderNonAudibleMultiTone::AnalyzeStartTokens");
  ...
  while (getSizeFilledBlockCircularBuffer() >= mSizeBlockCircularBuffer - 1) {
    ...
    if (firstTokenRepetitions >=
            ...
            2)))  // allow 10% tolerance for second start token
      {
        ...
        BDEBUG(
            "DecoderNonAudibleMultiTone::AnalyzeStartTokens FRONT-DOOR "
            "DETECTED first=%d second=%d",
...
int DecoderNonAudibleMultiTone::AnalyzeToken(float* audioBuffer) {
  BTRACE("DecoderNonAudibleMultiTone::AnalyzeToken decoding=%d", mDecoding);
  ...
  while (getSizeFilledBlockCircularBuffer() >=
    ...
    for (int i = 0; i < (mSizeBlockCircularBuffer / 2); i++) {
      ...
      // Statistics
      mBlockTokenStatistics[s].idxToken =
          Globals::getIdxTokenFromIdxsTonesNonAudibleMultiTone(
              mBlockTokenStatistics[s].idxToneMax,
              mBlockTokenStatistics[s].idxToneSecond);
      mBlockTokenStatistics[s].energyRatioToken =
          (mBlockTokenStatistics[s].energyRatioToneMax +
           mBlockTokenStatistics[s].energyRatioToneSecond) /
          2.f;
    }
    ...
    {
    int idx =
    mBlockEnergyRatiosTokenIdx[(mReadPosInBlockCircularBuffer+i)%mSizeBlockCircularBuffer];
...
int DecoderNonAudibleMultiTone::ComputeStatsStartTokens() {
  BTRACE("DecoderNonAudibleMultiTone::ComputeStatsStartTokens");
...
int DecoderNonAudibleMultiTone::ComputeStats() {
  BTRACE("DecoderNonAudibleMultiTone::ComputeStats");
  // energy mean in the alphabet frequency region
  ...
  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++) {
    ...
    mBlockTokenStatistics[s].energyToken =
  ...
  /*  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++)
    ...
    Globals::getIdxTokenFromIdxsTonesNonAudibleMultiTone(mBlockEnergyRatiosMaxToneIdx[s],
...
float DecoderNonAudibleMultiTone::ComputeBlockMagSpecSumsLastToken(
...
float DecoderNonAudibleMultiTone::ComputeBlockMagSpecSumsCurrentToken(
    int midFreqBin, int width, int nbins, std::vector<float>& sumPerFrame) {
  float mean = 0;
  float sumFr = 0;
