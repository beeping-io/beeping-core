...
EncoderHiddenMultiTone::EncoderHiddenMultiTone(float samplingRate, int buffsize,
                                               int windowSize)
    : Encoder(samplingRate, buffsize, windowSize, Globals::numTokensHidden,
...
int EncoderHiddenMultiTone::EncodeDataToAudioBuffer(const char* stringToEncode,
  ...
  for (int i = 0; i < digits.size(); i++) {
    ...
    Globals::getFreqsFromIdxHiddenMultiTone(digits[i], mSampleRate, mWindowSize,
                                            (float**)&mCurrentFreqs);
