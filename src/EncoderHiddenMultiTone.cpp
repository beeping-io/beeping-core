...
EncoderHiddenMultiTone::EncoderHiddenMultiTone(const BeepingConfig& config,
  ...
  BTRACE(
      "EncoderHiddenMultiTone::ctor sr={:.1f} buf={} win={} tokens={} tones={}",
...
int EncoderHiddenMultiTone::EncodeDataToAudioBuffer(const char* stringToEncode,
                                                    int type, int size,
                                                    const char* melodyString,
                                                    int melodySize) {
  BINFO("EncoderHiddenMultiTone::Encode payload=\"{}\" len={} type={}",
  ...
  BDEBUG("EncoderHiddenMultiTone::Encode RS encoded {} digits",
         (int)digits.size());
  ...
  for (int i = 0; i < static_cast<int>(digits.size()); i++) {
    ...
    Globals::getFreqsFromIdxHiddenMultiTone(
        digits[i], mSampleRate, mWindowSize,
        m_config.freqOffsetForHiddenMultiTone, (float**)&mCurrentFreqs);
    Globals::getLoudnessHiddenMultiToneFromIdx(digits[i],
    ...
    if (i > 1)  // after the start tokens
    {
      ...
      {
        mCurrentFreqs[0] += m_config.freqOffsetForHiddenMultiTone;
        mCurrentFreqs[1] += m_config.freqOffsetForHiddenMultiTone;
      } else if ((n % 3) == 2)  // Take care, encoder and decoder do not share
                                // same odd/even order if you compare mDecoding
                                // with digits index
      {
        mCurrentFreqs[0] += m_config.freqOffsetForHiddenMultiTone * 2;
        mCurrentFreqs[1] += m_config.freqOffsetForHiddenMultiTone * 2;
      }
    }
    /*
    ...
    */
    BTRACE(
        "EncoderHiddenMultiTone::Encode digit[{}]={} freq1={:.2f} freq2={:.2f}",
  ...
  BDEBUG("EncoderHiddenMultiTone::Encode totalSamples={}",
      ...
      for (int t = 0; t < samplesPerDigit; t++) {
        ...
        mAudioBufferEncodedString[t + i * samplesPerDigit] =
            ...
            (vol * factor * sin(pos * 2.f * BeepingConfig::pi));
                         ...
                         // mNumSamplesEncodedString
  {
    BDEBUG("EncoderHiddenMultiTone::Encode applying melody, melodySize={}",
           melodySize);
          ...
          // of size mNumSamplesEncodedString
  {
    ...
    BDEBUG("EncoderHiddenMultiTone::Encode applied audio signature {} samples",
           sizeToFill);
