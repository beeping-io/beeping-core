...
Encoder::Encoder(float samplingRate, int buffsize, int windowSize,
  ...
  mNumMaxSamplesEncodedString =
      (int)(10.f * samplingRate);  // max 10 seconds TODO: check this size
...
/*
int Encoder::EncodeDataToAudioBuffer(const char *stringToEncode, int type, int
size, const char *melodyString, int melodySize)
{
...
"EncodeDataToAudioBuffer %s type %d size %d", stringToEncode, type, size );
  ...
  digits.push_back(Globals::getIdxFromChar(Globals::frontDoorTokens[0]));
//front-door symbols
  digits.push_back(Globals::getIdxFromChar(Globals::frontDoorTokens[1]));
//front-door symbols
...
//  __android_log_print(ANDROID_LOG_INFO, "BeepingCoreLibInfo", "Added digits");
  ...
  int checkDigit = 0;
  for (int
i=Globals::numFrontDoorTokens;i<(Globals::numFrontDoorTokens+Globals::numWordTokens);i++)
    checkDigit += digits[i];
...
//__android_log_print(ANDROID_LOG_INFO, "BeepingCoreLibInfo", "Digit %d", i);
    ...
    int samplesForFadeBegin = (int)(mSampleRate *
Globals::durToken*Globals::durFade); int samplesForFadeEnd = (int)(mSampleRate *
Globals::durToken*tailLength); int samplesForGap = (int)(mSampleRate *
Globals::durToken*gapLength); float currentFreq =
Globals::getFreqFromIdx(digits[i],mSampleRate,mWindowSize); float
currentFreqLoudness = Globals::getLoudnessFromIdx(digits[i]);
    ...
    for (int t=0;t<samplesPerDigit;t++)
    {
      ...
      if (i==0)
      {
        samplesForFadeEnd = (int)(mSampleRate *
Globals::durToken*(tailLength*0.5f)); //for first token samplesForGap = 0;
      }
...
(t<(samplesPerDigit-samplesForGap)) ) factor = factor *
(float)((samplesPerDigit-samplesForGap)-t)/(float)(samplesForFadeEnd); else if
(t>=(samplesPerDigit-samplesForGap)) factor = 0.f;

//      __android_log_print(ANDROID_LOG_INFO, "BeepingCoreLibInfo", "Sample %d",
t);

      mAudioBufferEncodedString[t+i*samplesPerDigit] = currentFreqLoudness *
factor * sinf((2.f*Globals::pi*currentFreq)*((float)t/(float)mSampleRate));

    }
    mNumSamplesEncodedString+=samplesPerDigit;
  }

  mReadIndexEncodedAudioBuffer = 0; //New audio has been created, then reset
read index
...
mNumSamplesEncodedString
  {
    ...
    {
      int samplesPerDigit = (int)(mSampleRate * Globals::durToken);
      int samplesForFade = (int)(mSampleRate *
Globals::durToken*Globals::durFade);
...
mAudioBufferEncodedString[t+i*samplesPerDigit]) + (0.25f * factor * sin(pos
* 2.f * Globals::pi));
      }
...
Globals::durToken*Globals::durFade);
                  ...
                  float currentFreq =
Globals::getMusicalNoteFromIdx(melodyDigits[i]);
      ...
      for (int t=0;t<samplesPerDigit;t++)
      {
        ...
        mAudioBufferEncodedString[t+i*samplesPerDigit] = (0.75f *
mAudioBufferEncodedString[t+i*samplesPerDigit]) + (0.25f * factor *
sinf((2.f*Globals::pi*currentFreq)*((float)t/(float)mSampleRate)));
      }
...
int Encoder::GetEncodedAudioBuffer(float* audioBuffer) {
  ...
  for (int i = 0; i < mBufferSize; i++) {
    if (mReadIndexEncodedAudioBuffer >= mNumSamplesEncodedString) break;
