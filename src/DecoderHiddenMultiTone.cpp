...
DecoderHiddenMultiTone::DecoderHiddenMultiTone(const BeepingConfig& config,
  ...
  Globals::getIdxsFromIdxHiddenMultiTone(idxFrontDoorToken1,
                                         &idxTonesFrontDoorToken1);
  idxTonesFrontDoorToken2 = new int[2];
  Globals::getIdxsFromIdxHiddenMultiTone(idxFrontDoorToken2,
  ...
  mDecodingMode =
      Globals::/*DECODING_MODE::*/ DECODING_MODE_HIDDEN;  // 0=AUDIBLE,
                                                          ...
                                                          // 2=HIDDEN, 3=CUSTOM
  BTRACE(
      "DecoderHiddenMultiTone::ctor sr={:.1f} buf={} win={} tokens={} tones={}",
      samplingRate, buffSize, windowSize, config.numTokensHidden,
      config.numTonesHiddenMultiTone);
  BDEBUG("DecoderHiddenMultiTone::ctor freq2Bin={:.6f} beginBin={} endBin={}",
...
int DecoderHiddenMultiTone::getSizeFilledFrameCircularBuffer() {
  return Decoder::getSizeFilledFrameCircularBuffer();
}

int DecoderHiddenMultiTone::getSizeFilledBlockCircularBuffer() {
...
int DecoderHiddenMultiTone::DecodeAudioBuffer(float* audioBuffer, int size) {
  BTRACE("DecoderHiddenMultiTone::DecodeAudioBuffer size={} decoding={}", size,
         mDecoding);
  ...
  while (getSizeFilledFrameCircularBuffer() >=
    ...
    if (mDecoding == 0) {
      ...
      if (ret == 1) {
        ...
        BTRACE("DecoderHiddenMultiTone::DecodeAudioBuffer -> {}", -2);
                                             ...
                                             // now start decoding
    {
      ...
      if (ret >= 0) {
        ...
        mDecoding++;
        BDEBUG(
            "DecoderHiddenMultiTone::DecodeAudioBuffer token decoded idx={} "
            "char='{}'",
            ret, Globals::getCharFromIdx(ret));
        BTRACE("DecoderHiddenMultiTone::DecodeAudioBuffer -> {}", ret);
        return ret;
      }
    } else if (mDecoding >
                                           ...
                                           // complete word
    {
      ...
      BTRACE("DecoderHiddenMultiTone::DecodeAudioBuffer -> {}", -3);
...
int DecoderHiddenMultiTone::GetDecodedData(char* stringDecoded) {
  int messageOk = 1;
  ...
  BINFO("DecoderHiddenMultiTone::GetDecodedData -> rc={}", result);
...
int DecoderHiddenMultiTone::AnalyzeToken(float* audioBuffer) {
  BTRACE("DecoderHiddenMultiTone::AnalyzeToken decoding={}", mDecoding);
  ...
  while (getSizeFilledBlockCircularBuffer() >=
    ...
    for (int i = 0; i < (mSizeBlockCircularBuffer / 2); i++) {
      ...
      mBlockTokenStatistics[s].idxToken =
          Globals::getIdxTokenFromIdxsTonesHiddenMultiTone(
    ...
    int max = Globals::getIdxTokenFromIdxsTonesHiddenMultiTone(max1, max2);
...
float DecoderHiddenMultiTone::ComputeBlockMagSpecSumsLastToken(
...
float DecoderHiddenMultiTone::ComputeBlockMagSpecSumsCurrentToken(
    int midFreqBin, int width, int nbins, std::vector<float>& sumPerFrame) {
  float mean = 0;
  float sumFr = 0;
