...
SpectralAnalysis::SpectralAnalysis(Mode _mode, int _fftSize, int _windowSize,
                                   int _hopSize)
    : mode(_mode) {
  mFftSize = _fftSize;
...
inline int SpectralAnalysis::multiplyBuffers(float* tgtPtr,
                                             const float* srcPtrA,
                                             const float* srcPtrB, int n) {
...
inline int SpectralAnalysis::copyToBuffer(float* tgtPtr, const float* srcPtr,
                                          int n) {
  float* endPtr = tgtPtr + n;
...
inline void SpectralAnalysis::generateBlackmanHarris92Window(float* window,
                                                             int size) {
  int i;
  float fSum = 0;
  /* for -92dB */
  ...
  for (i = 0; i < size; i++) {
    fSum += window[i] = a0 - a1 * std::cos(fConst * i) +
                        a2 * std::cos(fConst * 2 * i) -
                        a3 * std::cos(fConst * 3 * i);
  }
...
inline void SpectralAnalysis::generateBlackmanHarris74Window(float* window,
                                                             int size) {
  int i;
  float fSum = 0;
  for (i = 0; i < size; i++)
...
inline void SpectralAnalysis::generateBlackmanHarrisWindow(float* window,
                                                           int size) {
