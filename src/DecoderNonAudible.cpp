...
DecoderNonAudible::DecoderNonAudible(const BeepingConfig& config,
  ...
  for (int i = 0; i < mNumTones; i++)
    mFreqsBins[i] =
        (int)(Globals::getFreqFromIdxNonAudible(i, mSampleRate, mWindowSize) *
                  mFreq2Bin +
              .5);

  // Optimize size of block spectrogram (only needed bins in token space range)
  mBeginBin =
      (int)(Globals::getFreqFromIdxNonAudible(0, mSampleRate, mWindowSize) *
                mFreq2Bin +
            .5);
  mEndBin = (int)(Globals::getFreqFromIdxNonAudible(mNumTokens - 1, mSampleRate,
                                                    mWindowSize) *
                      mFreq2Bin +
                  .5);
...
int DecoderNonAudible::AnalyzeStartTokens(
    float* audioBuffer) {  // float *magSpectrum, float* realSpectrum, float*
                           // imagSpectrum
  BTRACE("DecoderNonAudible::AnalyzeStartTokens");
  ...
  // int idx_b = Globals::maxValueIdx(mEnergyStd,Globals::numFreqs); //not being
  // used (but don't delete) int idx_c =
  // Globals::maxValueIdx(mEnergyDiff,Globals::numFreqs); //not being used (but
  // don't delete)
...
int DecoderNonAudible::ComputeStatsStartTokens() {
  BTRACE("DecoderNonAudible::ComputeStatsStartTokens");
...
int DecoderNonAudible::ComputeStats() {
  BTRACE("DecoderNonAudible::ComputeStats");
  // energy mean in the alphabet frequency region
  ...
  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++) {
    for (int i = 0; i < mNumTokens; i++) {
      ...
      if (size > 0) {
        if (i == mDecodedValues[size - 1])
          for (int n = 0; n < mSizeTokenBinAnal; n++) mEvalTokenMags[n] *= 0.2f;

        if ((size > 1) && (i == mDecodedValues[size - 2]))
          for (int n = 0; n < mSizeTokenBinAnal; n++) mEvalTokenMags[n] *= 0.4f;

        if ((size > 2) && (i == mDecodedValues[size - 3]))
          for (int n = 0; n < mSizeTokenBinAnal; n++) mEvalTokenMags[n] *= 0.7f;
      } else if (mDecoding > 0) {
    ...
    // Globals::secondValueIdx(mEnergyRatios,mNumTokens); //not being used (but
    // don't delete) int idx_maxEnergyStd =
    // Globals::maxValueIdx(mEnergyStd,mNumTokens); //not being used (but don't
    // delete) int idx_maxEnergyDiff =
    // Globals::maxValueIdx(mEnergyDiff,mNumTokens); //not being used (but don't
    // delete)
...
int DecoderNonAudible::DeReverbToken(const int nbins, int* freqsBins) {
  ...
  int binwidth = 1;  // bins taken into account around the center frequency to
                     // sum the energy
...
float DecoderNonAudible::ComputeBlockMagSpecSumsLastToken(
...
float DecoderNonAudible::ComputeBlockMagSpecSumsCurrentToken(
    int midFreqBin, int width, int nbins, std::vector<float>& sumPerFrame) {
  float mean = 0;
  float sumFr = 0;
