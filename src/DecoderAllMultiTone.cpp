...
DecoderAllMultiTone::DecoderAllMultiTone(const BeepingConfig& config,
                                         float samplingRate, int buffSize,
                                         int windowSize)
    : Decoder(config, samplingRate, buffSize, windowSize, config.numTokensAll,
              config.numTonesAll) {
#ifdef _IOS_LOG_
  ...
  memset(mBlockEnergyRatiosMaxToneIdx, -1,
         mSizeBlockCircularBuffer * sizeof(int));

  mBlockEnergyRatiosSecondToneIdx = new int[mSizeBlockCircularBuffer];
  memset(mBlockEnergyRatiosSecondToneIdx, -1,
         mSizeBlockCircularBuffer * sizeof(int));
  ...
  mBlockTokenStatisticsArray =
      new sTokenProbs*[NUM_SIMULTANEOUS_DECODING_MODES];
  ...
  for (int t = 0; t < NUM_SIMULTANEOUS_DECODING_MODES; t++) {
    ...
    if (t == Globals::/*DECODING_MODE::*/ DECODING_MODE_NONAUDIBLE) {
      ...
      Globals::getIdxsFromIdxNonAudibleMultiTone(
      ...
      for (int i = 0; i < mNumTones; i++)
        mFreqsBinsArray[t][i] = (int)(Globals::getToneFromIdxAudibleMultiTone(
                                          i, mSampleRate, mWindowSize) *
                                          mFreq2Bin +
                                      .5);
      // Optimize size of block spectrogram (only needed bins in token space
      // range)
      mBeginBinArray[t] = (int)(Globals::getToneFromIdxAudibleMultiTone(
                                    0, mSampleRate, mWindowSize) *
                                    mFreq2Bin +
                                .5);
      mEndBinArray[t] = (int)(Globals::getToneFromIdxAudibleMultiTone(
                                  mNumTones - 1, mSampleRate, mWindowSize) *
                                  mFreq2Bin +
                              .5);
  ...
  BTRACE("DecoderAllMultiTone::ctor sr=%.1f buf=%d win=%d tokens=%d tones=%d",
         samplingRate, buffSize, windowSize, config.numTokensAll,
         config.numTonesAll);
  BDEBUG("DecoderAllMultiTone::ctor freq2Bin=%.6f modes=%d", mFreq2Bin,
         NUM_SIMULTANEOUS_DECODING_MODES);
}

DecoderAllMultiTone::~DecoderAllMultiTone(void) {
  BTRACE("DecoderAllMultiTone::dtor");
  // delete[] mFreqsBins;
...
int DecoderAllMultiTone::DecodeAudioBuffer(float* audioBuffer, int size) {
  BTRACE("DecoderAllMultiTone::DecodeAudioBuffer size=%d decoding=%d", size,
         mDecoding);
  int sizeWindow = mSpectralAnalysis->mWindowSize;

  int i;
  for (i = 0; i < size; i++) {
    mCircularBufferFloat[(i + mWritePosInFrameCircularBuffer) %
                         (mSizeFrameCircularBuffer)] = audioBuffer[i];
  }
  ...
  while (getSizeFilledFrameCircularBuffer() >=
    ...
    for (i = 0; i < sizeWindow; i++)
      mAnalBufferFloat[i] =
          mCircularBufferFloat[(i + mReadPosInFrameCircularBuffer) %
                               (mSizeFrameCircularBuffer)];
    // advance readpos (advance hopsize instead of full buffersize)
    // mReadPosInFrameCircularBuffer =
    // (mReadPosInFrameCircularBuffer+mBufferSize)%(mSizeFrameCircularBuffer);
    mReadPosInFrameCircularBuffer =
        (mReadPosInFrameCircularBuffer + mSpectralAnalysis->mHopSize) %
        (mSizeFrameCircularBuffer);
    ...
    if (mDecoding == 0) {
      ...
      if (ret >= 1) {
        ...
        BINFO(
            "DecoderAllMultiTone::DecodeAudioBuffer START TOKEN DETECTED "
            "mode=%d",
            mDecodingMode);
        ...
        BTRACE("DecoderAllMultiTone::DecodeAudioBuffer -> %d", -2);
        return -2;  //-2 means start token found
      }
    } else if ((mDecoding > 0) &&
               (mDecoding <=
                Globals::numMessageTokens))  // we already found start token,
                                             // now start decoding
    {
      ...
      if (ret >= 0) {
        ...
        mDecoding++;
        BDEBUG(
            "DecoderAllMultiTone::DecodeAudioBuffer token decoded idx=%d "
            "char='%c' mode=%d",
            ret, Globals::getCharFromIdx(ret), mDecodingMode);
        BTRACE("DecoderAllMultiTone::DecodeAudioBuffer -> %d", ret);
        return ret;
      }
    } else if (mDecoding >
    ...
    {
      mDecoding = 0;
      mConfidenceEnergyRatios =
          mConfidenceEnergyRatios / Globals::numMessageTokens;
      mConfidenceRepetitions =
          mConfidenceRepetitions / Globals::numMessageTokens;
      mConfidenceRepetitions = (mConfidenceRepetitions / 2.f) + 0.5f;

      mReceivedBeepsVolume = mReceivedBeepsVolume / Globals::numMessageTokens;

      BINFO("DecoderAllMultiTone::DecodeAudioBuffer WORD COMPLETE mode=%d",
            mDecodingMode);
      BTRACE("DecoderAllMultiTone::DecodeAudioBuffer -> %d", -3);
...
int DecoderAllMultiTone::GetDecodedData(char* stringDecoded) {
  ...
  mReedSolomon->msg_len =
      mMessageLength;  // messagelength=(2 frontdoor + 9/10 digits + 1/0
                       // correction code (if any))

  // save check token
  int checkTokenReceived =
      mDecodedValues[Globals::numFrontDoorTokens + Globals::numWordTokens];

  int sizeTokensNoCorrection =
      Globals::numTotalTokens - Globals::numCorrectionTokens;
  ...
  int count = 0;
  for (int i = 0; i < sizeTokensNoCorrection; i++) {
    if (mDecodedValues[i] == mDecodedValuesOrig[i]) count++;
  }
  ...
  // PRINT DEBUG STATISTICS
  std::cout << "                 "
            << "  [Prob (correction):" << mConfidenceCorrection << "]"
            << std::endl;
#endif  // DEBUG_OUTPUT
  ...
  if (checkToken != checkTokenReceived) messageOk = -1;
  ...
  for (int i = Globals::numFrontDoorTokens; i < len; ++i) {
    mDecodedString[i - Globals::numFrontDoorTokens] =
        Globals::getCharFromIdx(mDecodedValues[i]);
  }

  mDecodedString[len - Globals::numFrontDoorTokens] =
      '\0';  // Set terminating character, although buffer was already
             // initialized with zeros
  strncpy(stringDecoded, mDecodedString,
          (len - Globals::numFrontDoorTokens) +
              1);  // to include terminating character

  // mDecodedString[0] = '\0';
  memset(mDecodedString, 0,
         MAX_DECODE_STRING_SIZE *
  ...
  BINFO("DecoderAllMultiTone::GetDecodedData -> rc=%d", result);
...
int DecoderAllMultiTone::AnalyzeStartTokens(
    float* audioBuffer) {  // float *magSpectrum, float* realSpectrum, float*
                           // imagSpectrum
  BTRACE("DecoderAllMultiTone::AnalyzeStartTokens");
  mSpectralAnalysis->doFFT(audioBuffer, mSpectralAnalysis->mSpecMag,
                           mSpectralAnalysis->mSpecPhase);
  memcpy(
      mBlockSpecMag[mWritePosInBlockCircularBuffer % mSizeBlockCircularBuffer],
      mSpectralAnalysis->mSpecMag,
      mSpectralAnalysis->mSpecSize *
          sizeof(float));  // needed because when starting decoding we need past
                           // spectrogram

  // computeStats
  for (int t = 0; t < NUM_SIMULTANEOUS_DECODING_MODES; t++) {
    // if (t>0) //to avoid advancement while in loop
    //   mReadPosInBlockCircularBuffer = (mReadPosInBlockCircularBuffer - 1 +
    //   mSizeBlockCircularBuffer) % (mSizeBlockCircularBuffer);
    ...
    for (c = 0; c < mNumTones - 1; c++) {
      for (d = 0; d < mNumTones - c - 1; d++) {
        if (mEnergyRatiosSorted[d] <
            mEnergyRatiosSorted[d + 1])  // For increasing order use >
        {
    ...
    sprintf(
        text,
        "mBlockEnergyRatios[%d]: %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
        mSizeBlockCircularBuffer, mBlockEnergyRatiosTokenIdx1Array[t][0],
        mBlockEnergyRatiosTokenIdx1Array[t][1],
        mBlockEnergyRatiosTokenIdx1Array[t][2],
        mBlockEnergyRatiosTokenIdx1Array[t][3],
        mBlockEnergyRatiosTokenIdx1Array[t][4],
        mBlockEnergyRatiosTokenIdx1Array[t][5],
        mBlockEnergyRatiosTokenIdx1Array[t][6],
        mBlockEnergyRatiosTokenIdx1Array[t][7],
        mBlockEnergyRatiosTokenIdx1Array[t][8],
        mBlockEnergyRatiosTokenIdx1Array[t][9],
        mBlockEnergyRatiosTokenIdx1Array[t][10],
        mBlockEnergyRatiosTokenIdx1Array[t][11],
        mBlockEnergyRatiosTokenIdx1Array[t][12],
        mBlockEnergyRatiosTokenIdx1Array[t][13],
        mBlockEnergyRatiosTokenIdx1Array[t][14],
        mBlockEnergyRatiosTokenIdx1Array[t][15]);
    ...
    if (t == 0)  // only the for the first mode
      mWritePosInBlockCircularBuffer =
          (mWritePosInBlockCircularBuffer + 1) % (mSizeBlockCircularBuffer);

#ifdef _ANDROID_LOG_
    char text2[150];
    sprintf(text2, "sizeFilled: %d | r:%d w:%d",
            getSizeFilledBlockCircularBuffer(), mReadPosInBlockCircularBuffer,
            mWritePosInBlockCircularBuffer);
    __android_log_write(ANDROID_LOG_INFO, "BeepingCoreLibInfo",
    ...
    while (getSizeFilledBlockCircularBuffer(t) >=
           mSizeBlockCircularBuffer - 1) {
      ...
      if (firstTokenRepetitions >=
          ((mSizeBlockCircularBuffer / 2) - mnToleranceFrames)) {
        int secondTokenRepetitions = 0;
        for (int i = (mSizeBlockCircularBuffer / 2);
             i < mSizeBlockCircularBuffer; i++) {
          int posIdx = (mReadPosInBlockCircularBufferArray[t] + i) %
                       mSizeBlockCircularBuffer;
        ...
        if (secondTokenRepetitions >=
            ((mSizeBlockCircularBuffer / 2) -
             (mnToleranceFrames *
              2)))  // allow 10% tolerance for second start token
        {
          // advance buffer read pos
          mReadPosInBlockCircularBufferArray[t] =
              mWritePosInBlockCircularBuffer;
          mEndStartTokenPosInBlockCircularBuffer =
              mReadPosInBlockCircularBufferArray[t];
          mAccumulatedDecodingFrames = 0.0;
          BDEBUG(
              "DecoderAllMultiTone::AnalyzeStartTokens FRONT-DOOR DETECTED "
              "mode=%d first=%d second=%d",
              t, firstTokenRepetitions, secondTokenRepetitions);
          return 1 + t;
        } else {
          // if (t >= (NUM_SIMULTANEOUS_DECODING_MODES-1))
          mReadPosInBlockCircularBufferArray[t] =
              (mReadPosInBlockCircularBufferArray[t] + 1) %
              (mSizeBlockCircularBuffer);
        }
      } else {
        // if (t >= (NUM_SIMULTANEOUS_DECODING_MODES - 1))
...
int DecoderAllMultiTone::AnalyzeToken(float* audioBuffer, int mode) {
  BTRACE("DecoderAllMultiTone::AnalyzeToken decoding=%d mode=%d", mDecoding,
         mode);
  ...
  memcpy(
      mBlockSpecMag[mWritePosInBlockCircularBuffer % mSizeBlockCircularBuffer],
      mSpectralAnalysis->mSpecMag,
      mSpectralAnalysis->mSpecSize * sizeof(float));
  // mBlockSpecMag

  mWritePosInBlockCircularBuffer =
      (mWritePosInBlockCircularBuffer + 1) % (mSizeBlockCircularBuffer);

  while (getSizeFilledBlockCircularBuffer(mode) >=
         (mSizeBlockCircularBuffer /
          2)) {  // we have a new filled block of (durToken size)
    ...
    // DeReverbToken(nbins, mFreqsBins); //not needed anymore, now we are
    // varying the base freq between odd and even
    ...
    for (int i = 0; i < (mSizeBlockCircularBuffer / 2); i++) {
      int s = (mReadPosInBlockCircularBufferArray[mode] + i) %
              mSizeBlockCircularBuffer;
      ...
      mBlockTokenStatisticsArray[mode][s].energyRatioToken =
          (mBlockTokenStatisticsArray[mode][s].energyRatioToneMax +
           mBlockTokenStatisticsArray[mode][s].energyRatioToneSecond) /
          2.f;
    }
...
#ifdef DEBUG_OUTPUT
    // PRINT DEBUG STATISTICS
    std::cout << "  [DEBUG_OUTPUT] " << "Token: " << max << " Prob: "
              << (((float)mToneRepetitions[max1] +
                   (float)mToneRepetitions[max2]) /
                  (float)mSizeBlockCircularBuffer)
              << " max1:" << mToneRepetitions[max1]
              << " max2:" << mToneRepetitions[max2] << std::endl;
    std::cout << "                 " << "  Prob2:" << prob << std::endl;
    std::cout << "                 " << "  Prob3 (combined):" << prob_combined
              << std::endl;
    std::cout << "                 " << "  energy:"
              << 20 * log10(energy / (float)(mSizeBlockCircularBuffer / 2))
              << std::endl;
#endif  // DEBUG_OUTPUT
    // END NEW METHOD
    ...
    // Update statistics
    mConfidenceRepetitions +=
        ((float)mToneRepetitions[max1] + (float)mToneRepetitions[max2]) /
        (float)mSizeBlockCircularBuffer;
    ...
    //remove reverb
    //int reverbFrames = (int)( ((float)mSizeBlockCircularBuffer / 2.f) * 0.10f
    + .5f); //was 0.25 int reverbFrames = 0; for (int
    i=reverbFrames;i<(mSizeBlockCircularBuffer/2);i++)
    {
    int idx =
    mBlockEnergyRatiosTokenIdx1Array[t][(mReadPosInBlockCircularBuffer+i)%mSizeBlockCircularBuffer];
    ...
    //PRINT DEBUG STATISTICS
    std::cout << "  [DEBUG_OUTPUT] " << "Token: " << max << " Prob: " <<
    ((float)mTokenRepetitions[max]/((float)mSizeBlockCircularBuffer/2.f)) <<
    "max:" << mTokenRepetitions[max] << std::endl; #endif //DEBUG_OUTPUT
    ...
    */
    ...
    mReadPosInBlockCircularBufferArray[mode] =
        (mEndStartTokenPosInBlockCircularBuffer + offset) %
        mSizeBlockCircularBuffer;
    // compute thresholds based on senitivity parameter. TODO: apply this??
    /*    float sensitivity = 0.5; // the higher the more sensivity has the
    algorithm. [0..-1] float ratiosThres = std::min(-18.f, std::max(-26.f, -26.f
    + (1.f-sensitivity)*12.f)); // range [-26..-18] def -20 float diffThres =
    std::min(-6.f, std::max( -30.f, -30.f + (1.f-sensitivity)*24.f)); // range
    [-13..-7] def -10 if ( (maxEnergyRatios > ratiosThres) && (maxEnergyDiff >
    diffThres) )
    {
...
int DecoderAllMultiTone::ComputeStatsStartTokens(int mode) {
  BTRACE("DecoderAllMultiTone::ComputeStatsStartTokens mode=%d", mode);
  ...
  energyBlock =
      energyBlock / (float)(mEndBinArray[mode] - mBeginBinArray[mode] + 1);
  ...
  for (int i = 0; i < mNumTones; i++) {
    ...
    for (int n = 0; n < mSizeTokenBinAnal; n++) {
      mEvalToneMags[n] =
          mSpectralAnalysis->mSpecMag[evalToneBin - mBinWidth + n];
      // mEvalToneMags[n] = mBlockSpecMag[(mReadPosInBlockCircularBuffer + t) %
      // mSizeBlockCircularBuffer][evalToneBin - mBinWidth + n];
    }
    ...
    mEnergyRatios[i] =
        20.0 * log10((energyTone) / energyBlock);  // difference of 3dB
  }
...
int DecoderAllMultiTone::ComputeStats(int mode) {
  BTRACE("DecoderAllMultiTone::ComputeStats mode=%d", mode);
  // energy mean in the alphabet frequency region
  ...
  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++) {
    for (int i = mBeginBinArray[mode]; i <= mEndBinArray[mode]; i++) {
      energyBlock +=
          mBlockSpecMag[(mReadPosInBlockCircularBufferArray[mode] + t) %
                        mSizeBlockCircularBuffer][i];
    }
  }
  energyBlock =
      energyBlock / ((double)(mEndBinArray[mode] - mBeginBinArray[mode] + 1) *
                     (mSizeBlockCircularBuffer / 2));
  ...
  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++) {
    for (int i = 0; i < mNumTones; i++) {
      ...
      //CURRENT TOKEN
      int evalTokenBin1 =
      (int)(Globals::getToneFromIdxNonAudibleMultiTone(mIdxs[0], mSampleRate,
      mWindowSize) * mFreq2Bin + .5); int evalTokenBin2 =
      (int)(Globals::getToneFromIdxNonAudibleMultiTone(mIdxs[1], mSampleRate,
      mWindowSize) * mFreq2Bin + .5);
      */
      ...
      mEnergy[i] = (square_sum * mSpectralAnalysis->mSpecSize) /
                   (float)(mSizeBlockCircularBuffer / 2);
      mEnergyRatios[i] =
          20.0 * log10((energyToken) / energyBlock);  // difference of 3dB
    }

    // float maxEnergyRatios = Globals::maxValue(mEnergyRatios,mNumTokens);
    // float maxEnergyStd = Globals::maxValue(mEnergyStd,mNumFreqs); //not being
    // used (but don't delete) float maxEnergyDiff =
    // Globals::maxValue(mEnergyDiff,mNumFreqs); //not being used (but don't
    // delete)

    int idx_maxEnergyRatios = Globals::maxValueIdx(mEnergyRatios, mNumTones);
    int idx_secondEnergyRatios =
        Globals::secondValueIdx(mEnergyRatios, mNumTones);

    // int idx_maxEnergyRatios2 =
    // Globals::secondValueIdx(mEnergyRatios,mNumFreqs); //not being used (but
    // don't delete) int idx_maxEnergyStd =
    // Globals::maxValueIdx(mEnergyStd,mNumFreqs); //not being used (but don't
    // delete) int idx_maxEnergyDiff =
    // Globals::maxValueIdx(mEnergyDiff,mNumFreqs); //not being used (but don't
    // delete)

    // mBlockEnergyRatiosTokenIdx1Array[t][(mReadPosInBlockCircularBuffer+t) %
    // mSizeBlockCircularBuffer] = idx_maxEnergyRatios;
    int s = (mReadPosInBlockCircularBufferArray[mode] + t) %
            mSizeBlockCircularBuffer;
    mBlockEnergyRatiosMaxToneIdx[s] = idx_maxEnergyRatios;
    mBlockEnergyRatiosSecondToneIdx[s] = idx_secondEnergyRatios;

    // mBlockEnergyRatiosTokenIdx2Array[t][(mReadPosInBlockCircularBuffer+t) %
    // mSizeBlockCircularBuffer] = idx_maxEnergyRatios2; //not being used (but
    // don't delete) mBlockEnergyStdTokenIdx[(mReadPosInBlockCircularBuffer+t) %
    // mSizeBlockCircularBuffer] = idx_maxEnergyStd; //not being used (but don't
    // delete) mBlockEnergyDiffTokenIdx[(mReadPosInBlockCircularBuffer+t) %
    // mSizeBlockCircularBuffer] = idx_maxEnergyDiff; //not being used (but
    // don't delete)  }
    ...
    mBlockTokenStatisticsArray[mode][s].energyRatioToneMax =
        mEnergyRatios[idx_maxEnergyRatios];
    mBlockTokenStatisticsArray[mode][s].energyRatioToneSecond =
        mEnergyRatios[idx_secondEnergyRatios];
    mBlockTokenStatisticsArray[mode][s].energyToken =
        mEnergy[idx_maxEnergyRatios] + mEnergy[idx_secondEnergyRatios];
  }
  ...
  /*  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++)
    ...
    mSizeBlockCircularBuffer;
      ...
      int energyRatioTokenIdx =
    Globals::getIdxTokenFromIdxsTonesHiddenMultiTone(mBlockEnergyRatiosMaxToneIdx[s],
    mBlockEnergyRatiosSecondToneIdx[s]);
...
float DecoderAllMultiTone::ComputeBlockMagSpecSumsLastToken(
  ...
  for (int i = 0; i < (mSizeBlockCircularBuffer / 2); i++) {
    ...
    for (int j = lbin; j < rbin; j++) {
      sumFr += mBlockSpecMag[(mReadPosInBlockCircularBuffer +
                              (mSizeBlockCircularBuffer / 2) + i) %
                             mSizeBlockCircularBuffer]
                            [j];  // accumulate linear magnitude
    }
    mean += sumFr;
...
float DecoderAllMultiTone::ComputeBlockMagSpecSumsCurrentToken(
    int midFreqBin, int width, int nbins, std::vector<float>& sumPerFrame) {
  float mean = 0;
  float sumFr = 0;
  ...
  for (int i = 0; i < (mSizeBlockCircularBuffer / 2); i++) {
    sumFr = 0;
    for (int j = lbin; j < rbin; j++) {
      sumFr += mBlockSpecMag[(mReadPosInBlockCircularBuffer + i) %
                             mSizeBlockCircularBuffer]
                            [j];  // accumulate linear magnitude
    }
    mean += sumFr;
