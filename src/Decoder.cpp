...
Decoder::Decoder(const BeepingConfig& config, float samplingRate, int buffSize,
  ...
  mDecoding = 0;
  mDecodingMode = -1;  // Mode between enum DECODING_MODE { NONAUDIBLE = 0,
                       // AUDIBLE = 1, HIDDEN = 2, CUSTOM = 3 };
  mDecodedString[0] = '\0';
  ...
  mSpectralAnalysis = new SpectralAnalysis(
      ...
      mHopSize);  // fftsize = 2048, windowsize=2048, hopsize = 512
  ...
  mBinWidth = 1;  // is 2, maybe should be changed to 1 for closer freqs (non
                  // audible multitone)
  // mBinWidth = 1;
  ...
  mEnergyRatios =
      new float[mNumTones];  // should be mNumTones!! first fix all the code
  ...
  memset(mBlockEnergyRatiosTokenIdx, -1,
  ...
  memset(mBlockEnergyRatiosTokenIdx2, -1,
         mSizeBlockCircularBuffer * sizeof(int));
  ...
  memset(mBlockEnergyRatiosTokenIdx3, -1,
         mSizeBlockCircularBuffer * sizeof(int));

  mBlockEnergyRatiosTokenIdx4 = new int[mSizeBlockCircularBuffer];
  memset(mBlockEnergyRatiosTokenIdx4, -1,
         mSizeBlockCircularBuffer * sizeof(int));

  // mFirstTokenBinOffsetInBlock = mBinWidth + mSizeNighbBins + 1; //2 binWidth
  // + 1 gap bin + 4 neighbours mBeginBinBlock = mBeginBin -
  // mFirstTokenBinOffsetInBlock; mEndBinBlock = mEndBin +
  // mFirstTokenBinOffsetInBlock;
  ...
  mMessageLength = Globals::numFrontDoorTokens + Globals::numWordTokens +
                   1;  // 12 = 2 + 9 + 1 // messagelength=(2 frontdoor + 9
                       // digits) + correction code
...
int Decoder::getSizeFilledFrameCircularBuffer() {
  ...
  if (mWritePosInFrameCircularBuffer >= mReadPosInFrameCircularBuffer)
    ...
    sizeFilled = mWritePosInFrameCircularBuffer +
...
int Decoder::getSizeFilledBlockCircularBuffer() {
  ...
  if (mWritePosInBlockCircularBuffer >= mReadPosInBlockCircularBuffer)
    ...
    sizeFilled = mWritePosInBlockCircularBuffer +
                 (mSizeBlockCircularBuffer - mReadPosInBlockCircularBuffer);
  return sizeFilled;
}

int Decoder::getSizeFilledBlockCircularBuffer(int mode) {
  int sizeFilled = 0;
  if (mWritePosInBlockCircularBuffer >=
    ...
    sizeFilled =
        ...
        (mSizeBlockCircularBuffer - mReadPosInBlockCircularBufferArray[mode]);
  return sizeFilled;
}
...
float Decoder::GetDecodingBeginFreq() { return 0.f; }

float Decoder::GetDecodingEndFreq() { return 0.f; }

int Decoder::GetSpectrum(float* spectrumBuffer) {
  memcpy(spectrumBuffer, mSpectralAnalysis->mSpecMag,
         (mSpectralAnalysis->mFftSize / 2) * sizeof(float));
...
int Decoder::ComputeStats() { return -1; }
