...
DecoderAudibleMultiTone::DecoderAudibleMultiTone(const BeepingConfig& config,
  ...
  for (int i = 0; i < mNumTones; i++)
    mFreqsBins[i] = (int)(Globals::getToneFromIdxAudibleMultiTone(
                              i, mSampleRate, mWindowSize) *
                              mFreq2Bin +
                          .5);

  // Optimize size of block spectrogram (only needed bins in token space range)
  mBeginBin = (int)(Globals::getToneFromIdxAudibleMultiTone(0, mSampleRate,
                                                            mWindowSize) *
                        mFreq2Bin +
                    .5);
  mEndBin = (int)(Globals::getToneFromIdxAudibleMultiTone(
  ...
  Globals::getIdxsFromIdxAudibleMultiTone(idxFrontDoorToken1,
                                          &idxTonesFrontDoorToken1);
  idxTonesFrontDoorToken2 = new int[2];
  Globals::getIdxsFromIdxAudibleMultiTone(idxFrontDoorToken2,
  ...
  mDecodingMode =
      Globals::/*DECODING_MODE::*/ DECODING_MODE_AUDIBLE;  // 0=AUDIBLE,
                                                           ...
                                                           // 2=HIDDEN, 3=CUSTOM
  BTRACE(
      "DecoderAudibleMultiTone::ctor sr={:.1f} buf={} win={} tokens={} "
      "tones={}",
      samplingRate, buffSize, windowSize, config.numTokensAudible,
      config.numTonesAudibleMultiTone);
  BDEBUG("DecoderAudibleMultiTone::ctor freq2Bin={:.6f} beginBin={} endBin={}",
...
int DecoderAudibleMultiTone::DecodeAudioBuffer(float* audioBuffer, int size) {
  BTRACE("DecoderAudibleMultiTone::DecodeAudioBuffer size={} decoding={}", size,
         mDecoding);
  ...
  while (getSizeFilledFrameCircularBuffer() >=
                                           ...
                                           // complete word
    {
      ...
      BTRACE("DecoderAudibleMultiTone::DecodeAudioBuffer -> {}", -3);
...
int DecoderAudibleMultiTone::GetDecodedData(char* stringDecoded) {
  ...
  BINFO("DecoderAudibleMultiTone::GetDecodedData -> rc={}", result);
...
int DecoderAudibleMultiTone::AnalyzeToken(float* audioBuffer) {
  BTRACE("DecoderAudibleMultiTone::AnalyzeToken decoding={}", mDecoding);
  ...
  while (getSizeFilledBlockCircularBuffer() >=
    ...
    for (int i = 0; i < (mSizeBlockCircularBuffer / 2); i++) {
      ...
      mBlockTokenStatistics[s].idxToken =
          Globals::getIdxTokenFromIdxsTonesAudibleMultiTone(
    ...
    /*    for (int i = 0; i<mNumTokens; i++)
        ...
        //remove reverb
        //int reverbFrames = (int)( ((float)mSizeBlockCircularBuffer / 2.f) *
      0.10f + .5f); //was 0.25 int reverbFrames = 0; for (int
...
int DecoderAudibleMultiTone::ComputeStats() {
  ...
  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++) {
    for (int i = 0; i < mNumTones; i++) {
      ...
      //CURRENT TOKEN
      int evalTokenBin1 =
      (int)(Globals::getToneFromIdxAudibleMultiTone(mIdxs[0], mSampleRate,
      mWindowSize) * mFreq2Bin + .5); int evalTokenBin2 =
      (int)(Globals::getToneFromIdxAudibleMultiTone(mIdxs[1], mSampleRate,
  ...
  /*  for (int t = 0; t < (mSizeBlockCircularBuffer / 2); t++)
    ...
    Globals::getIdxTokenFromIdxsTonesAudibleMultiTone(mBlockEnergyRatiosMaxToneIdx[s],
...
float DecoderAudibleMultiTone::ComputeBlockMagSpecSumsLastToken(
...
float DecoderAudibleMultiTone::ComputeBlockMagSpecSumsCurrentToken(
    int midFreqBin, int width, int nbins, std::vector<float>& sumPerFrame) {
  float mean = 0;
  float sumFr = 0;
