...
ReedSolomon::ReedSolomon(void) {
  ...
  // kk = 9;           /* kk = nn-2*tt */
  msg_len =
      12;  // Voctro: we are using Shortened RS Code, which means we are
           // encoding/decoding less symbols. Voctro: for Shortened-RS, we will
           // should zero-pad from position 12 to 23 in the message array.
  ...
  // For decode
  elp = new int*[nn - kk + 2];
  for (int i = 0; i < (nn - kk + 2); i++) elp[i] = new int[nn - kk];
  d = new int[nn - kk + 2];
  l = new int[nn - kk + 2];
...
ReedSolomon::~ReedSolomon(void) {
  ...
  for (int i = 0; i < (nn - kk + 2); i++) delete[] elp[i];
  delete[] elp;
...
void ReedSolomon::Decode() {
  ...
  if (syn_error) /* if errors, try and correct */
  {
    ...
    'mu'th discrepancy, where u='mu'+1 and 'mu' (the Greek letter!) is the step
    number ranging from -1 to 2*tt (see L&C),  l[u] is the degree of the elp at
    that step, and u_l[u] is the difference between the step number and the
    degree of the elp.
    */
...
void ReedSolomon::SetMessage(const std::vector<int> message) {
  ...
  for (int i = 0; i < msg_len; ++i) data[i] = message[i];

  // pad with zeros for Shortened RS code.
  if (msg_len < kk) memset(data + msg_len, 0, (kk - msg_len) * sizeof(int));
}

void ReedSolomon::GetCode(std::vector<int>& code) {
  code.clear();
  // fill with message values
  for (int i = 0; i < msg_len; ++i) code.push_back(data[i]);
  // fill with correction code stored in bb[0:7]
  for (int i = 0; i < (nn - kk); ++i) code.push_back(bb[i]);
...
void ReedSolomon::GetMessage(std::vector<int>& message) {
  message.clear();
  // fill with message values
  for (int i = 0; i < msg_len; ++i) message.push_back(recd[(nn - kk) + i]);
}
