...
DecoderHiddenMultiTone::DecoderHiddenMultiTone(float samplingRate, int buffSize, int windowSize) : Decoder(samplingRate, buffSize, windowSize, Globals::numTokensHidden, Globals::numTonesHiddenMultiTone)
{
#ifdef _IOS_LOG_
  ...
  for (int i = 0; i<mNumTones; i++)
    mFreqsBins[i] = (int)(Globals::getToneFromIdxHiddenMultiTone(i, mSampleRate, mWindowSize) * mFreq2Bin + .5);

  //Optimize size of block spectrogram (only needed bins in token space range)
  mBeginBin = (int)(Globals::getToneFromIdxHiddenMultiTone(0, mSampleRate, mWindowSize) * mFreq2Bin + .5);
  mEndBin = (int)(Globals::getToneFromIdxHiddenMultiTone(mNumTones - 1, mSampleRate, mWindowSize) * mFreq2Bin + .5);
  ...
  Globals::getIdxsFromIdxHiddenMultiTone(idxFrontDoorToken1, &idxTonesFrontDoorToken1);
  idxTonesFrontDoorToken2 = new int[2];
  Globals::getIdxsFromIdxHiddenMultiTone(idxFrontDoorToken2, &idxTonesFrontDoorToken2);

  mDecodingMode = Globals::/*DECODING_MODE::*/DECODING_MODE_HIDDEN; //0=AUDIBLE, 1=NONAUDIBLE, 2=HIDDEN, 3=CUSTOM
}
...
int DecoderHiddenMultiTone::AnalyzeStartTokens(float *audioBuffer)
...
int DecoderHiddenMultiTone::AnalyzeToken(float *audioBuffer)
{
  ...
  { //we have a new filled block of (durToken size)
    ...
    for (int i = 0; i<(mSizeBlockCircularBuffer / 2); i++)
    {
	  ...
	  //Statistics
	  mBlockTokenStatistics[s].idxToken = Globals::getIdxTokenFromIdxsTonesHiddenMultiTone(mBlockTokenStatistics[s].idxToneMax, mBlockTokenStatistics[s].idxToneSecond);
...
float DecoderHiddenMultiTone::ComputeBlockMagSpecSumsLastToken(int midFreqBin, int width, int nbins, std::vector<float> &sumPerFrame)
{
  float mean = 0;
  float sumFr = 0;
...
float DecoderHiddenMultiTone::ComputeBlockMagSpecSumsCurrentToken(int midFreqBin, int width, int nbins, std::vector<float> &sumPerFrame)
{
  float mean = 0;
  float sumFr = 0;
